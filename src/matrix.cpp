...
/**
 * @brief Helper function to check if submatrix is antisymmetric.
 * @return true if submatrix is antisymmetric otherwise returns false.
*/
bool Matrix::checkAntiSymmetry() {
    logger.log("Matrix::checkAntiSymmetry");
    vector<vector<int> > submatrix1;
    vector<vector<int> > submatrix2;
    ...
    for (int pageRow = 0; pageRow < this->pagesCountInRow; pageRow++) {
        for (int pageCol = pageRow; pageCol < this->pageCountInColumn; pageCol++) {
            if (pageRow == pageCol) {
                ...
                // Check diagonal elements, they must be zero for antisymmetry
                for (int i = 0; i < submatrix1.size(); i++) {
                    if (submatrix1[i][i] != 0)
                        return false;
                }
                continue;
            ...
            // Check antisymmetry condition: submatrix1[i][j] == -submatrix2[j][i]
            if (!this->checkSubmatrixAntisymmetry(submatrix1, submatrix2)) 
                return false;
        }
    }
 ...
 * @return true if submatrix is antisymmetric otherwise returns false.
*/
bool Matrix::checkSubmatrixAntisymmetry(vector<vector<int > > submatrix1, vector<vector<int > > submatrix2) {
    logger.log("Matrix::checkSubmatrixAntisymmetry");
