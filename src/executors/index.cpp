...
//         table.indexed = true;
//         cout << "Table name: " << table.tableName << "\n";
//         cout << "Column name: " << table.columns[colIndex] << "\n";
//         cout << "Column index: " << colIndex << "\n";
//         table.indexedColumn = colIndex;
...
void executeINDEX()
{
    ...
    // Get a pointer to the actual table in the catalogue (not a copy!)
    Table* table = tableCatalogue.getTable(parsedQuery.indexRelationName);
    ...
    if (parsedQuery.indexingStrategy == HASH) {
        ...
        for (const auto& pair : pushvals) {
            table->hashtable->insert(pair);
        }
    }
